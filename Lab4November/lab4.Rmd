...
```{r}
    ...
    curve(dgamma(x, shape=a.round, scale=s.round),add=T, col='red')
```
...
### 9.1 Отдельно
```{r}
  hist(cats$Hwt[cats$Sex=='M'], col = 'blue',probability = T, xlab="Male Hwt", main="Density Male HWt")
  curve(dgamma(x, shape=Male[1], scale=Male[2]),add=T, lwd=2)
```

```{r}
  hist(cats$Hwt[cats$Sex=='F'],probability = T,col='red',xlab="Female Hwt", main="Density Female HWt")
  curve(dgamma(x, shape=Female[1], scale=Female[2]),add=T,lwd=2)
```
...
```{r}
  ...
  curve(dgamma(x, shape=Male[1], scale=Male[2]),add=T, lwd=2,col='blue')
  curve(dgamma(x, shape=Female[1], scale=Female[2]),add=T,lwd=2,col='red')
```
...
```{r echo=FALSE}
    ...
    test.fun()
```
> При разделении списками, происходит сдвиг среднего в сторону преобладающих показателей M or F. При разделении случайным образом, показатель среднего каждой группы находиться около среднего значения для всех котов.
